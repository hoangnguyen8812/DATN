...
namespace BaiTapLon.Areas.Admin.Controllers
{
    public class ThongKeController : BaseController
    {
        ...
        public ActionResult ExportExel(string fromDate, string toDate)
        {
            ...
            var wb = new XLWorkbook(@"C:\Users\namdx\Desktop\DATN\Do_An\BaiTapLon\Resource\Template\Tempalet_Doanh_Thu.xlsx");
            var workSheet = wb.Worksheet(1);
            if (!string.IsNullOrEmpty(fromDate) || !string.IsNullOrEmpty(toDate))
            {
                workSheet.Cell("C1").Value = "Thống Kê Doanh Thu";
            }
            else
            {
                workSheet.Cell("C1").Value = "Thống Kê Doanh Thu Cả Năm";
            }
